...
- 測定IDは `G-ZR46K2ME6D` を使用する。
- 別ストリームID `G-65GV6BS65C` をコード、HTML、Netlify設定で使用しない。
- `gtag('config', 'G-ZR46K2ME6D')` を重複実行しない。
...
- `googletagmanager.com/gtag/js?id=G-ZR46K2ME6D` が1回だけ読み込まれる。
- GA4の収集リクエストが送信される。
...
- Google Tag Assistantで `G-ZR46K2ME6D` が検出される。
