...
公開中のGASはサイトのデプロイでは更新されない。既存のWebアプリ配置を「新しいバージョン」で更新し、`/exec` のURLを変えないこと。
更新前に名簿のバックアップを取り、テスト送信は実際の個人情報を使わず、保存行と通知メールを両方確認する。
改善版では既存の10列の後ろに `submission_id` と `notification_status` を追加する。`failed` または `pending` の行は通知メールが届いていない可能性があるため、名簿を定期的に確認する。
